...
class SOLUTION:
    ...
    def Wait_For_Simulation_To_End(self):
        ...
        oscillation_file = f"oscillations{str(self.myID)}.txt"
        while not os.path.exists(oscillation_file):
            time.sleep(0.01)

        oscillation_file = open(oscillation_file, "r")
        self.oscillation = float(oscillation_file.read())
        oscillation_file.close()

        os.system(f"rm oscillations{str(self.myID)}.txt")
